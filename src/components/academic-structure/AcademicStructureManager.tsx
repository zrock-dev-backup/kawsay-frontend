...
const AcademicStructureManager: React.FC = () => {
    ...
    const handleAddGroup = (cohortId: number, groupName: string) => addStudentGroup(cohortId, groupName);
    ...
    if (error) {
        return <Alert severity="error" sx={{m: 2}}>An error occurred: {error}</Alert>;
    }
