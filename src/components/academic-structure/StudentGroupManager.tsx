...
const StudentGroupManager: React.FC<StudentGroupManagerProps> = ({cohort, onAddGroup, onAddSection}) => {
    ...
    return (
        <Box sx={{width: '100%', pl: 4, pr: 2, py: 1}}>
            <Typography variant="subtitle1" sx={{fontWeight: 'bold'}}>Groups</Typography>
            ...
            {cohort.studentGroups.map(group => (
                <Accordion key={group.id} TransitionProps={{unmountOnExit: true}}
                           sx={{my: 1, boxShadow: 1, '&:before': {display: 'none'}}}>
                    <AccordionSummary expandIcon={<ExpandMoreIcon/>}>
                        <Typography sx={{width: '50%', flexShrink: 0}}>{group.name}</Typography>
                        <Typography
                            sx={{color: 'text.secondary'}}>{`ID: ${group.id} | Sections: ${group.sections.length}`}</Typography>
                    </AccordionSummary>
                    <AccordionDetails sx={{p: 0}}>
                        <SectionManager group={group} cohortId={cohort.id} onAddSection={onAddSection}/>
                    </AccordionDetails>
                </Accordion>
            ))}
