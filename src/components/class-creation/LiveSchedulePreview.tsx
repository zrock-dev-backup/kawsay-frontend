...
export const LiveSchedulePreview: React.FC<Props> = ({wizard}) => {
    ...
    const teacherSchedule = useMemo(() => {
        ...
        const teacherClasses = existingClasses.filter(c => c.teacherDto?.id === wizard.state.form.teacherId);
    ...
    return (
        <Paper elevation={2} sx={{p: 2, height: '100%'}}>
            <Typography variant="h6" gutterBottom>Live Schedule Preview</Typography>
            <Typography variant="body2" color="text.secondary">
                Teacher: {wizard.state.qualifiedTeachers.find(t => t.id === wizard.state.form.teacherId)?.name || 'Not Selected'}
            </Typography>
            <Typography variant="body2" color="text.secondary">
