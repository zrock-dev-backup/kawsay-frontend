...
const SlotPicker: React.FC<SlotPickerProps> = ({
  ...
  const handleSlotClick = (dayId: number, periodId: number) => {
    const isSelected = value.some(
      (slot) => slot.dayId === dayId && slot.startPeriodId === periodId,
    );
    let newValue;
    if (isSelected) {
  ...
  const periodIdToIndexMap = useMemo(
    () => new Map(periods.map((p, i) => [p.id, i])),
    [periods],
  );
  ...
  return (
    <SlotPickerContainer>
      <FilterContainer>
        ...
        >
          <ToggleButton value="all" aria-label="all day">
            All
          </ToggleButton>
          ...
          return (
            <React.Fragment key={period.id}>
              ...
              {days.map((day) => {
                const isSelected = value.some(
                  (slot) =>
                    slot.dayId === day.id && slot.startPeriodId === period.id,
                );
                return (
                  <SlotButton
                    ...
                    title={
                      isSelectionDisabled
                        ? `Cannot start a ${length}-period class here.`
                        : `${day.name} at ${period.start}`
                    }
                  />
                );
              })}
