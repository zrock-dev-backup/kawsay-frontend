...
import {createClass, fetchClassById, updateClass} from '../../services/apiClassService.ts';
import {fetchCourses, fetchTimetableStructureById} from '../../services/apiService.ts';
...
const getInitialFormState = (timetableId: number, classType: ClassType): ClassFormState => ({
    id: null,
...
export function useClassCreationWizard(timetableId: string) {
    const [state, setState] = useState<WizardState>({
        isLoading: true, isSubmitting: false, fetchError: null, submitStatus: null, step: 0,
        timetableStructure: null, allCourses: [], classTypeConfigs: new Map(), qualifiedTeachers: [],
    ...
    const setFormValue = useCallback(<K extends keyof ClassFormState>(key: K, value: ClassFormState[K]) => {
        setState(prev => ({
            ...prev,
    ...
    const validateForm = (): boolean => {
        ...
        if (!form.length || form.length <= 0) errors.length = 'Length must be greater than 0.';
        if (!form.frequency || form.frequency <= 0) errors.frequency = 'Frequency must be at least 1.';

        setState(prev => ({...prev, validationErrors: errors}));
        return Object.keys(errors).length === 0;
    };

    const initializeForMode = useCallback(async (classId: number | null, classType: ClassType) => {
        setState(prev => ({...prev, isLoading: true, submitStatus: null, validationErrors: {}}));
        ...
        if (classId) { // EDIT MODE
            try {
                ...
                const teachers = await getQualifiedTeachersForCourse(classToEdit.courseId);
                setState(prev => ({
                    ...prev,
                    ...
                    form: {
                        ...
                        startDate: classToEdit.startDate ? dayjs(classToEdit.startDate) : null,
                ...
                setState(prev => ({...prev, fetchError: "Failed to load class data for editing.", isLoading: false}));
            }
        } else { // CREATE MODE
            ...
            setState(prev => ({
                ...
                form: {
                    ...
                    startDate: prev.timetableStructure ? dayjs(prev.timetableStructure.startDate) : null,
                    endDate: prev.timetableStructure ? dayjs(prev.timetableStructure.startDate).add(8, 'week') : null,
                },
                isLoading: false,
            }));
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            const apiCall = id ? updateClass(id, requestData) : createClass(requestData);
            const result = await apiCall;
            const message = id ? `Class "${result.courseName}" updated.` : `Class "${result.courseName}" created.`;
            setState(prev => ({...prev, submitStatus: {type: 'success', message}}));
            return true; // Signal success to parent
        } catch (err) {
            const message = err instanceof Error ? err.message : "An unknown error occurred.";
            setState(prev => ({...prev, submitStatus: {type: 'error', message}}));
            return false;
        } finally {
    ...
    return {state, initializeForMode, setFormValue, handleCourseSelect, handleStep, handleSubmit};
}
