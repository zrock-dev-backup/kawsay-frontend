...
export const courseHandlers = [
  ...
  http.get(`${COURSES_URL}/summary`, async () => {
    ...
    const courseSummary = db.courses.map((c) => ({
      id: c.id,
      name: `${c.name} (${c.code})`, // Format for display
      code: c.code,
    }));
  ...
  http.get(`${COURSES_URL}/:courseId/qualified-teachers`, async ({ params }) => {
    const courseId = Number(params.courseId);
    console.log(`[MSW] Fetching qualified teachers for courseId: ${courseId}`);
    await delay(150);
